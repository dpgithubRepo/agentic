...
class WebSummarizer():
    ...
    def web_summarizer(self) -> Agent:
        return Agent(
            config=self.agents_config['web_summarizer'], # type: ignore[index]
            verbose=True
        )
    ...
    @task
    def web_summarizer_task(self) -> Task:
        return Task(
            config=self.tasks_config['web_summarizer_task'], # type: ignore[index]
            output_file='report.md'
        )

    @crew
    def crew(self) -> Crew:
        """Creates the WebSummarizer crew"""
